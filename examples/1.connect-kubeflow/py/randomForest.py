...
with kfp_server_api.ApiClient(configuration, cookie=auth_session["session_cookie"]) as api_client:
    ...
    description="This is a only_randomForest_pso pipline."
    try:
    ...
    display_name = 'run_only_randomForest(pso)' + random_suffix()
    description = "This is a only_randomForest(pso) run."
        ...
        for node_id, node in nodes.items():
                if 'inputs' in node and 'parameters' in node['inputs']:
                    for parameter in node['inputs']['parameters']:
                        if parameter['name'] == 'particle-swarm-optimization-Accuracy': #change parameter
                            output_value = parameter['value']

    if output_value is not None:
        print(f"Random Forest (PSO) Classifier Accuracy: {output_value}")
    else:
